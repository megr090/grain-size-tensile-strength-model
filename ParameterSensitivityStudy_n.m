%% Test the sensitivity of the grain-size model to the flow law exponent n
...
elat = 1e-9;
ns = [2,3,4];
grainsize = zeros(length(z),length(ns));
T = zeros(length(z),length(ns));
for n = 1:length(ns)
    [T(:,n),grainsize(:,n),zetaaddiff] = computeGrainSizefromSR(elat,D,p,ns(n),theta,H,z);
end
...
subplot(1,2,1)
for i=1:length(ns)
    plot(T(:,i),z,'LineWidth',2,'Color',cmap(i,:))
    hold on
end
hold on
