%% Test the sensitivity of the grain-size model to the activation energies
...
grainsize = zeros(length(z),length(Qs));
for Q = 1:length(Qs)
    [T,grainsize(:,Q),zetaaddiff] = computeGrainSizefromSR(elat,D,p,n,theta,H,z,Qs(Q,2),Qs(Q,1),tc);
end
...
Qgp = Qs(1,1);
tc = -10;
[Qg,Qc,Qm] = defineActivationEnergies_Test(T_ex,Qgm,Qgp,tc);

figure;
plot(T_ex,Qg./1e3,'Color','k','LineWidth',2)
hold on
plot(T_ex,Qm./1e3,'Color','r','LineWidth',2)
plot(T_ex,Qc./1e3,'Color','b','LineWidth',2)
set(gca,'FontSize',18,'FontWeight','b','GridColor','r');
xlim([240,273])
%ylim([0 H])
xlabel('Temperature (K)')
ylabel('Activation Energy (kJ mol^{-1})')
grid on
legend({'$$ Q_{gg} $$','$$ Q_m $$','$$ Q_c $$'},'Interpreter','latex')
...
title('Grain Size (mm)')
grid on
legend({'$$ Q_{gg}^+ = 100, Q_{gg}^- = 40 $$','$$ Q_{gg}^+ = 80, Q_{gg}^- = 40 $$','$$ Q_{gg}^+ = 100, Q_{gg}^- = 60 $$'},'Interpreter','latex')
...
D = 0.05;
for Q=1:length(Qs)
    [grainsize(:,Q),viscosity,time_to_eq,tau] = findGrainSize_TestAE(T,f,strainrate,R,mu,D,M0,k0,gamma,p,c,n,Qs(Q,2),Qs(Q,1),tc);
end
...
title('Grain Size (mm)')
grid on
legend({'$$ Q_{gg}^+ = 100, Q_{gg}^- = 40 $$','$$ Q_{gg}^+ = 80, Q_{gg}^- = 40 $$','$$ Q_{gg}^+ = 100, Q_{gg}^- = 60 $$','GRIP'},'Interpreter','latex')
...
D = 0.05;
for Q=1:length(Qs)
    [grainsize_model(:,Q),viscosity,time_to_eq,tau] = findGrainSize_TestAE(T,f,strainrate,R,mu,D,M0,k0,gamma,p,c,n,Qs(Q,2),Qs(Q,1),tc);
end
...
title('Grain Size (mm)')
grid on
legend({'$$ Q_{gg}^+ = 100, Q_{gg}^- = 40 $$','$$ Q_{gg}^+ = 80, Q_{gg}^- = 40 $$','$$ Q_{gg}^+ = 100, Q_{gg}^- = 60 $$','GISP2'},'Interpreter','latex')
...
title('Grain Size (mm)')
grid on
legend({'$$ Q_{gg}^+ = 100, Q_{gg}^- = 40 $$','$$ Q_{gg}^+ = 80, Q_{gg}^- = 40 $$','$$ Q_{gg}^+ = 100, Q_{gg}^- = 60 $$','WAIS Divide'},'Interpreter','latex')
