...
grainsize_model = zeros(length(T),1);
grainsize_model = ((4.*mu.^2.*k0.*exp(-Qg./(R.*T)).*p.^(-1).*c.*gamma+tau.^4.*D.^(p).*(0.5.*p).*M0.*exp(-Qm./(R.*T)))./(4.*mu.^2.*tau.*(1-theta).*strainrate)).^(1/(1+p));
grainsize_model =  grainsize_model.*1e3;
...
figure;
plot(gisp2_age,gisp2_depth2,'r','LineWidth',2)
hold on
xlim([0 2.5e5])
ylim([0 H])
...
xlabel('Time (yr)')
ylabel('Depth (m)')
legend('GISP Age','Time to Equilibrium')
grid on
...
hold on
plot(grainsize_model,z,'b','LineWidth',2)
set(gca,'FontSize',18,'FontWeight','b','GridColor','r');
hold on
...
%ylabel('Height (m)') 
grid on
legend({'Model, this study','GISP2','location','northeast'},'FontSize',14)
