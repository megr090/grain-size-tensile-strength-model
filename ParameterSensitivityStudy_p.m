%% Test the sensitivity of the grain-size model to the grain growth exponent
...
% varying p
ps = [2,4,6,9];
grainsize = zeros(length(z),length(ps));
theta = 0.99;
D = 0.05;
elat = 1e-9;
n = 3;
for p = 1:length(ps)
    [T,grainsize(:,p),zetaaddiff] = computeGrainSizefromSR(elat,D,ps(p),n,theta,H,z);
end
...
for p=1:length(ps)
    ...
    k0 = k0./1000.^ps(p); %m^p/s
    [grainsize(:,p),tau,A] = findGrainSize(T,f,strainrate,R,mu,D,M0,k0,gamma,ps(p),c,n);
end
...
ps = [2,4,6,9];
grainsize_model = zeros(length(z),length(ps));
...
for p=1:length(ps)
    k0 = 11.4266; %mm^p/s (Azuma et al 2012) at Q=42 kJ/mol
    k0 = k0./1000.^ps(p); %m^p/s
    [grainsize_model(:,p),tau,A] = findGrainSize(T,f,strainrate,R,mu,D,M0,k0,gamma,ps(p),c,n);
end
...
title('Grain Size (mm)')
grid on
legend('p=2','p=4','p=6','p=9','WAIS Divide')
